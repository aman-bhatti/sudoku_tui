...
func (m GameModel) renderBoard() string {
	...
	for i := 0; i < sudokuLen; i++ {
		row := ""
		for j := 0; j < sudokuLen; j++ {
			// Correctly check if the current cell is in the error coordinates map
			isError := m.errCoordinates[coordinate{i, j}] // Make sure this is correctly referencing the coordinates

			// Debugging to confirm error logic
			fmt.Printf("Cell [%d, %d] isError: %v\n", i, j, isError)
